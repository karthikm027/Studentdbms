'''
Student database management system with Graphical User Inteface(GUI)
Sqlite3 is used for database management.
tkinter python module is used for GUI.
Users can create,update or delete a student tuple.
'''
...
c.execute("""CREATE TABLE IF NOT EXISTS students(    #student table is created if it does not exist already
usn text,
fname text,
sname text,
dob text,
...
def clear():    #To clear the entries from input fields
    usntxt.delete(0,END)
    fnametxt.delete(0,END)
    snametxt.delete(0,END)
    dobtxt.delete(0,END)
...
def delete(): #To delete a tuple. Type in the USN number and then click on DELETE
    con=sqlite3.connect("student_database.db")
    c=con.cursor()
...
def search():  #To search a student tuple, type in the USN number and click on SEARCH
    con=sqlite3.connect("student_database.db")
    c=con.cursor()
    if len(usn.get())!=0 and type(int(usn.get()))==int:
...
def update():  #To update the details of an existing tuple, fill all the fields and click UPDATE
    con=sqlite3.connect("student_database.db")
    c=con.cursor()
    if len(usn.get())!=0 and type(int(usn.get()))==int:
